...
namespace gli
{
	...
	sampler2d_array<T, P>::sampler2d_array(texture_type const & Texture, gli::wrap Wrap, filter Mip, filter Min, texel_type const & BorderColor)
	...
	template <typename T, qualifier P>
	typename sampler2d_array<T, P>::texture_type const & sampler2d_array<T, P>::operator()() const
	{
		return this->Texture;
	}

	template <typename T, qualifier P>
	typename sampler2d_array<T, P>::texel_type sampler2d_array<T, P>::texel_fetch(extent_type const & TexelCoord, size_type layer, size_type Level) const
	{
	...
	template <typename T, qualifier P>
	void sampler2d_array<T, P>::texel_write(extent_type const & TexelCoord, size_type layer, size_type Level, texel_type const & Texel)
	{
		GLI_ASSERT(!this->Texture.empty());
	...
	template <typename T, qualifier P>
	void sampler2d_array<T, P>::clear(texel_type const & Color)
	{
		GLI_ASSERT(!this->Texture.empty());
		GLI_ASSERT(this->Convert.Write);
	...
	template <typename T, qualifier P>
	typename sampler2d_array<T, P>::texel_type sampler2d_array<T, P>::texture_lod(normalized_type const & SampleCoord, size_type Layer, level_type Level) const
	{
		GLI_ASSERT(!this->Texture.empty());
	...
	template <typename T, qualifier P>
	void sampler2d_array<T, P>::generate_mipmaps(filter Minification)
	{
	...
	template <typename T, qualifier P>
	void sampler2d_array<T, P>::generate_mipmaps(size_type BaseLayer, size_type MaxLayer, size_type BaseLevel, size_type MaxLevel, filter Minification)
	{
		GLI_ASSERT(!this->Texture.empty());
