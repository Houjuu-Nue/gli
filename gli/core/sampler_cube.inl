...
namespace gli
{
	...
	sampler_cube<T, P>::sampler_cube(texture_cube const & Texture, gli::wrap Wrap, filter Mip, filter Min, texel_type const & BorderColor)
	...
	template <typename T, qualifier P>
	texture_cube const & sampler_cube<T, P>::operator()() const
	{
		return this->Texture;
	}

	template <typename T, qualifier P>
	typename sampler_cube<T, P>::texel_type sampler_cube<T, P>::texel_fetch(extent_type const & TexelCoord, size_type Face, size_type Level) const
	{
	...
	template <typename T, qualifier P>
	void sampler_cube<T, P>::texel_write(extent_type const & TexelCoord, size_type Face, size_type Level, texel_type const & Texel)
	{
		GLI_ASSERT(!this->Texture.empty());
	...
	template <typename T, qualifier P>
	void sampler_cube<T, P>::clear(texel_type const & Color)
	{
		GLI_ASSERT(!this->Texture.empty());
		GLI_ASSERT(this->Convert.Write);
	...
	template <typename T, qualifier P>
	typename sampler_cube<T, P>::texel_type sampler_cube<T, P>::texture_lod(normalized_type const & SampleCoord, size_type Face, level_type Level) const
	{
		GLI_ASSERT(!this->Texture.empty());
	...
	template <typename T, qualifier P>
	void sampler_cube<T, P>::generate_mipmaps(filter Minification)
	{
	...
	template <typename T, qualifier P>
	void sampler_cube<T, P>::generate_mipmaps(size_type BaseFace, size_type MaxFace, size_type BaseLevel, size_type MaxLevel, filter Minification)
	{
		GLI_ASSERT(!this->Texture.empty());
